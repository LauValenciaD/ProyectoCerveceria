...
$producto_id = $_SESSION["producto_id"] ?? '';
    ...
    mysqli_stmt_close($sentencia); // Cierra la sentecia
    // Si se pulsa el boton de borrar
    if (isset($_POST["submit"])) {
        if ($producto["Foto"] !== null) { // si tiene foto
        ...
        $actualizacion = mysqli_prepare($con, "DELETE FROM productos
                                        WHERE ID_PRODUCTO = ?");
        if ($actualizacion) {
            // Vincula los parámetros para la consulta preparada
            mysqli_stmt_bind_param($actualizacion, 'i', $producto_id);
            // Borra el producto y te lleva al catálogo
